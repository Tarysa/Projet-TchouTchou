...
public class RailCourbeHD extends Case{

	
	/**
	 * Constructeur de la classe RailCourbeHD
	 * @param p : Point qui correspond à l'endroit où se trouvera cet rail
	 */
	public RailCourbeHD(Point p) {
		super(p, 4);
	}
	
	/**
	 * Méthode centre_rotation permettant d'obtenir le centre de notre cercle
     * @return renvoie le point du centre de rotation du rail (ici haut droite du rail)
	 */
	@Override
	public Point centre_rotation(){
		
	    return new Point((int)m_p.getX() + 200, (int)m_p.getY());
	}
	 ...
	 * @return : renvoie 1 si l'objet t traverse la rail dans le sens haut -> droite (anti-horaire), 2 si l'objet t traverse le rail dans le sens droite -> haut (horaire), 0 s'il ne peut pas traverser
	 */
	@Override
